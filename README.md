...
[![Supabase](https://img.shields.io/badge/supabase-conectado-3ecf8e)](https://supabase.com)
...
| ✨ Match con tu CV | Una búsqueda por cada skill top de tu CV, todo rankeado por % |
| 🛡 Anti-fraude + anti-fantasma | Bloquea `bairesdev`/`manstaff`, carnadas "banco de talentos" y reposts +60 días |
| 🗄 Supabase | Cada búsqueda se guarda sola (upsert por `url`) |
| 🌐 ES/EN total + 🌙 modo oscuro | Toda la interfaz cambia, sin textos mezclados |
...
**2. En el navegador:** abre https://maurux01.github.io/workapp/ (tu CV nunca sale de tu equipo).
...
- Web local → `python main_web.py` (abre el navegador sola en http://localhost:5000)
...
3. Doble clic en una tarjeta para ver el **detalle completo**.
4. Cambia **ES/EN** o **🌙/☀️** cuando quieras; agrega empresas a bloquear con
   `BLOCKED_COMPANIES=otra1,otra2` en tu `.env`.

## 🗄 Supabase (opcional, 2 min)

1. Corre `supabase/schema.sql` en el **SQL Editor** (tabla `jobs` + RLS + índices).
2. Corre también: `grant select, insert, update, delete on public.jobs to anon, authenticated;`
3. Copia `SUPABASE_URL` y `SUPABASE_KEY` (**anon**, nunca `service_role`) a tu `.env`.
4. Sin keys la app funciona igual con caché local de 24 h.
...
```
main_desktop.py · main_web.py · Workapp.pyw/.bat   lanzadores
core/        cv_analyzer · job_matcher · spam_detector (anti-fantasma)
scrapers/    9 fuentes + manager (cache, dedupe, blocklist)
parsers/     pdf, limpieza HTML, tipos/modalidad
storage/     cache local + supabase · supabase/schema.sql
ui/          desktop tkinter (tema claro/oscuro) · web/ Flask local
index.html + assets/   sitio estático de GitHub Pages
```
...
- **¿Upwork?** Sin feed público (mató su RSS) — se cubre freelance vía Remotive/RemoteOK/WWR/HN.
- **¿Jooble/Indeed/Computrabajo fallan a veces?** Sí, ponen muro anti-bots (403/500); la app sigue con las demás fuentes + caché.
